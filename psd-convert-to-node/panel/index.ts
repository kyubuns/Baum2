...
Editor.Panel.extend({
  ...
  ready() {
    ...
    Editor.Ipc.sendToMain(
      "psd-convert-to-node:print-text",
      "psd-convert-to-node ready"
    );
    ...
    this.$pngfileElement.onchange = ev => {
      ...
      let basename = Path.basename(
        pngfileElement.files[0].path,
        Path.extname(pngfileElement.files[0].path)
      );
    ...
    this.$psd_convert_btn.addEventListener("confirm", () => {
      ...
      // @ts-ignore
      Editor.Ipc.sendToMain(
        "psd-convert-to-node:convet-psd-file-by-fullpath",
        [isReverse, isSaveJson, layoutPath, pngFolder],
        msg => {
          // @ts-ignore
          Editor.log(
            "psdからノードツリーに変換完了，msg:",
            msg,
            ",nodeName:",
            "Canvas"
          );
